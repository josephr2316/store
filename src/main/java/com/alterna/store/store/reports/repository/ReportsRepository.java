...
public interface ReportsRepository extends JpaRepository<com.alterna.store.store.orders.entity.OrderEntity, Long> {
	...
	/**
	 * Native query uses String for status to avoid PostgreSQL JDBC type mismatch with Java enum.
	 * Caller must pass OrderStatus.DELIVERED.name() (i.e. "DELIVERED").
	 */
	@Query(value = "SELECT CAST(o.created_at AS DATE) AS sale_date, COUNT(*), COALESCE(SUM(o.total_amount), 0) FROM orders o WHERE o.status = CAST(:status AS VARCHAR) AND o.created_at >= :from AND o.created_at < :to GROUP BY CAST(o.created_at AS DATE) ORDER BY sale_date", nativeQuery = true)
	List<Object[]> deliveredOrdersCountAndTotalByDay(@Param("from") Instant from, @Param("to") Instant to, @Param("status") String status);
